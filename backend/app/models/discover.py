...
class JoinResponse(BaseModel):
    ...
    # ADR 0014 — true when the request bubbled to the owner queue because
...
class PendingRequest(BaseModel):
    ...
    # ADR 0014 — denormalised onto the join-request doc so the owner
...
class MinorJoinRequest(BaseModel):
    """Owner-facing view of a minor's pending join-request (ADR 0014).
...
class OwnerApproveJoinRequest(BaseModel):
    """`POST /api/admin/groups/{gid}/join-requests/{uid}/approve` body (ADR 0014).
